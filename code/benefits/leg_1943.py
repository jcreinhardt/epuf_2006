...
def i1943(income_stream:list, index_year:int):
...
def a1943(income_stream:list, index_year:int, birth_year:int, retirement_year:int):
    ...
    total_wage=0
    count=0
    adjusted_income_stream=i1943(income_stream, index_year)
...
def q1943 (income_stream:list, index_year:int, birth_year:int): 
    """This function determines whether an individual is considered fully-insured as of the 1939 legislation.
        This function returns a boolean.
        The inputs are (adjusted_income_stream:list, index_year:int, birth_year:int).
        adjusted_income_stream -- a list of nominal incomes a person received in each year, adjusted by the relevant legislation. i1939 will create the caps for you.
...
def bb1943(income_stream:list, index_year:int, birth_year:int):
    ...
    avg_monthly_wage=a1943(income_stream, index_year, birth_year)
    adjusted_income_stream=i1943(income_stream, index_year)
...
def b1943(income_stream:list, index_year:int, birth_year:int):
    ...
    if q1943(adjusted_income_stream, index_year, birth_year)==True and retirement_age>=65:
        monthly_benefits=bb1943(income_stream, index_year, birth_year)
    
    return monthly_benefits 

#tax rules, found on first few lines of 1943 revenue act pdf
# "Freezing Pay-Roll Tax" https://www.dropbox.com/scl/fi/exi1eoj95f7wvzutzowtu/1943_revenue_act.pdfrlkey%3Dcjgnjjw2xf5zp5fhhkh6z0al2%26st%3Dl4jp5wwm%26dl%3D0?e=1
def t1943(income_stream:list, index_year:int):
    """This function outputs the nominal monthly benefit (int) an individual would have received under the 1943 legislation. 
        The function inputs are (income_stream:list, index_year:int, birth_year:int).
        income_stream -- a list of nominal incomes a person received in each year.
        index_year -- the year in which an individual begins their income_stream.
...
def death1943(income_stream:list, index_year:int, birth_year:int, retirement_year:int, death_year:int, survivors:bool):
    '''
    ...
    '''
    if (q1943(income_stream, index_year, birth_year) and not survivors): 
        monthlyBenefits = b1943(income_stream, index_year, birth_year, retirement_year)
        return 6 * monthlyBenefits
    else:
        return 0
    

def spouse1943(income_stream:list, index_year:int, birth_year:int, retirement_year:int, reference_year:int, woman:bool, spouse_income_stream:list, spouse_index_year:int, spouse_birth_year:int, spouse_retirement_year:int, spouse_reference_year:int, spouse_woman:bool):
    ...
    if spouse_woman == True and woman == False:
        spousal_benefit = 0
        spouseMonthlyBenefit = b1943(spouse_income_stream, spouse_index_year, spouse_birth_year, spouse_retirement_year)
        primaryPIA = bb1943(income_stream, index_year, birth_year)
...
def widow1943(income_stream:list, index_year:int, birth_year:int, retirement_year:int, reference_year:int, widow_income_stream:list, widow_index_year:int, widow_birth_year:int, widow_retirement_year:int, widow_reference_year:int, widow_woman:bool):
    """This function outputs the nominal monthly benefit (int) an individual would have received under the 1950 legislation. 
        The function inputs are (income_stream:list, index_year:int, birth_year:int).
        income_stream -- a list of nominal incomes a person received in each year.
        index_year -- the year in which an individual begins their income_stream.
    ...
    """ 
    if widow_woman == True and widow_retirement_year >= widow_birth_year + 65 and q1943(income_stream,index_year,birth_year):
        spousal_benefit = 0
        spouseMonthlyBenefit = b1943(widow_income_stream, widow_index_year, widow_birth_year)
        primaryPIA = bb1943(income_stream, index_year, birth_year)
...
def MFB1943(income_stream:list, index_year:int, birth_year:int, retirement_year:int, reference_year:int,death_year:int, woman:bool):
    """
    This function outputs the Maximum Family Benefit a principal beneficiary would be capped at under the relevant legislation.
     income_stream -- a list of nominal incomes a person received in each year.
        ...
        Reference year -- to which year would the dollars be indexed to. Likely the year in which the entitlement is being paid out.
    """
    primary_adjusted_income_stream=i1943(income_stream, index_year, retirement_year)
    primary_avg_monthly_wage = a1943(primary_adjusted_income_stream, index_year, birth_year, retirement_year,reference_year, woman)
    primaryMonthlyBenefit = b1943(primary_adjusted_income_stream, index_year, birth_year, retirement_year, reference_year, woman)
