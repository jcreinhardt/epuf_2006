...
def i1960(income_stream:list, index_year:int, retirement_year:int):
...
#there are changes, see page pdf page 37 of 1960, minimum is now two years to calculate an avg monthly wage
#the drop rule also needs to be added, sec 215(b)

def a1960(adjusted_income_stream:list, original_income_stream, index_year:int, birth_year:int, retirement_year:int, reference_year:int, woman:bool, death_year = 0):
    ...
    old_income_stream = adjusted_income_stream
...
###################
    ...
    beginning_year=1951
    QualYear = 1961
    relevantInct = adjusted_income_stream[0:QualYear - index_year]
    while not q1960(old_income_stream, original_income_stream, index_year, birth_year, reference_year, woman):
        #print("I am not eligible at ", nra)
        QualYear +=1
        relevantInct = adjusted_income_stream[0:QualYear - index_year]
    if death_year == 0:
        end_year = QualYear
    else:
        end_year = min(QualYear, death_year)
    #end_year=birth_year+65 #note that this is different from the previus legislation, "first entitled" now means age 62 in the legislation
    drops=5
...
###################
    ...
    if birth_year+22>1950:#the reasoning here is that there is a distinction in the legislation between those who turn 22 before and after 1950.
        ...
        beginning_year=1951
        #In the 1960 amendments, the end year is actually the first year after 1960 (not inclusive) that the individual becomes eligible for retirement.
...
###################
    ...
    return math.trunc(avg_monthly_wage)
...
#this was updated, see pdf page 25
def q1960(adjusted_income_stream:list, original_income_stream:list, index_year:int, birth_year:int,  retirement_year:int, woman:bool, skip1939 = False): #this might not be completely correct/ might be off by half a year
    ...
    if coverage_quarters>=40:
        return True
    elif ((3*(coverage_quarters)/4)>=((end_year)-reference_year)) and (coverage_quarters>=6):
        return True
    else:
        return False

def qcurrent1960(income_stream:list, index_year:int, birth_year:int, retirement_year:int, reference_year:int, death_year:int, woman:bool): 
...
def bb1960(adjusted_income_stream:list, original_income_stream:list, avg_monthly_wage:float, index_year:int, birth_year:int, retirement_year:int, reference_year:int, woman:bool):
    ...
    pib=0 
    pia_1954=0
    #avg_monthly_wage=a1960(adjusted_income_stream, index_year, birth_year)
    #print("average: ", avg_monthly_wage)
    ...
    # if ((q1960(adjusted_income_stream, index_year, birth_year)==True) and (birth_year+22>=1951))--for some reasons we kept adding these "ands" however they always fuck up the formula
    
    if ((q1960(adjusted_income_stream, original_income_stream, index_year, birth_year, reference_year, woman, skip1939 = True)==True)): #Check if the worker is fully-insured, as well as to place them in the proper formula
...
#total 
def b1960(income_stream:list, original_income_stream:list, index_year:int, birth_year:int, retirement_year:int, reference_year:int, woman:bool):
    ...
    """ 
    monthly_benefits=0
    adjusted_income_stream=i1960(income_stream, index_year, retirement_year)
    nominal_total_contributions=t1960(income_stream, index_year)
    ...
    else: avg_monthly_wage=a1960(adjusted_income_stream,original_income_stream,  index_year, birth_year, retirement_year, reference_year, woman)

    monthly_benefits=bb1960(adjusted_income_stream, original_income_stream, avg_monthly_wage, index_year, birth_year, retirement_year, reference_year, woman)
...
def t1960(income_stream:list, index_year:int):
    
    employee_tax_rate = 0
    employer_tax_rate = 0
    nominal_contributions=[]

    adjusted_income_stream=i1960(income_stream, index_year, index_year + len(income_stream)) #to apply the taxable maximums to the benefit stream
...
def death1960(income_stream:list, index_year:int, birth_year:int, retirement_year:int, reference_year:int,death_year:int, woman:bool):
    '''
    ...
    '''
    if (q1960(income_stream, index_year, birth_year, reference_year, woman)): 
        monthlyBenefits = b1960(income_stream, index_year, birth_year, retirement_year)
        return max(3 * monthlyBenefits, 255)
    else:
        return 0


def spouse1960(income_stream:list, index_year:int, birth_year:int, retirement_year:int, reference_year:int, woman:bool, spouse_income_stream:list, spouse_index_year:int, spouse_birth_year:int, spouse_retirement_year:int, spouse_reference_year:int, spouse_woman:bool):
    """This function outputs the nominal monthly benefit (int) an individual would have received under the 1950 legislation. 
        The function inputs are (income_stream:list, index_year:int, birth_year:int).
        income_stream -- a list of nominal incomes a person received in each year.
        index_year -- the year in which an individual begins their income_stream.
    ...
    spousal_benefit = 0
    if q1960(spouse_income_stream, spouse_index_year, spouse_birth_year, spouse_retirement_year, spouse_woman):
        spouse_benefit = b1960(spouse_income_stream, spouse_index_year, spouse_birth_year, spouse_retirement_year, spouse_reference_year,spouse_woman)
    else: spouse_benefit = 0
    primary_adjusted_income_stream=i1960(income_stream, index_year, retirement_year)
    primary_avg_monthly_wage = a1960(primary_adjusted_income_stream, index_year, birth_year, retirement_year,reference_year, woman)
    primary_PIA = bb1960(income_stream, primary_avg_monthly_wage, index_year, retirement_year, birth_year,reference_year, woman)
...
def widow1960(income_stream:list, index_year:int, birth_year:int, retirement_year:int, reference_year:int, death_year:int, woman:bool, widow_income_stream:list, widow_index_year:int, widow_birth_year:int, widow_retirement_year:int, widow_reference_year:int, widow_woman:bool):
    """This function outputs the nominal monthly benefit (int) an individual would have received under the 1950 legislation. 
        The function inputs are (income_stream:list, index_year:int, birth_year:int).
        income_stream -- a list of nominal incomes a person received in each year.
        index_year -- the year in which an individual begins their income_stream.
    ...
    """ 
    if ((widow_retirement_year >= widow_birth_year + 65) or (widow_woman == True and widow_retirement_year >= widow_birth_year +62)) and q1960(income_stream,index_year,birth_year, reference_year, woman):
        widow_benefit = 0
        if q1960(widow_income_stream,widow_index_year, widow_birth_year, widow_reference_year, widow_woman):
            widowMonthlyBenefit = b1960(widow_income_stream, widow_index_year, widow_birth_year,widow_retirement_year, widow_reference_year, widow_woman)
        else: widowMonthlyBenefit = 0
        primary_adjusted_income_stream=i1960(income_stream, index_year, retirement_year)
        primary_avg_monthly_wage = a1960(primary_adjusted_income_stream, index_year, birth_year, retirement_year,reference_year, woman, death_year)
    
        primaryPIA = bb1960(income_stream,primary_avg_monthly_wage, index_year, birth_year, retirement_year, reference_year, woman)
...
def MFB1960(income_stream:list, index_year:int, birth_year:int, retirement_year:int, reference_year:int,death_year:int, woman:bool):
    """
...
def children1960(income_stream:list, index_year:int, birth_year:int, retirement_year:int, reference_year:int,death_year:int, woman:bool, child_birth_year:int):
    """
    ...
    if reference_year < death_year:
        ...
        primary_avg_monthly_wage = a1960(primary_adjusted_income_stream, index_year, birth_year, retirement_year,reference_year, woman, death_year)
    primary_PIA = bb1960(income_stream, primary_avg_monthly_wage, index_year, birth_year, retirement_year,reference_year, woman)
