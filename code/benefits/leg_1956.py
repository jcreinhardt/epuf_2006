...
#only other changes from 1954 are in tax schedules, and that we now drop 5 years
...
def i1956(income_stream:list, index_year:int, retirement_year:int):
...
#the legislation can be found on page 11 and 12 of 1954 pdf. It doesn't seem like there are any changes from 1954, so it might require a closer look
#the drop rule also needs to be added, sec 215(b)
def a1956(adjusted_income_stream:list, original_income_stream:list, index_year:int, birth_year:int, retirement_year:int, reference_year:int, woman:bool):
...
###################
    ...
    while not q1956(qualificationInc, original_income_stream, index_year, birth_year, retirement_year, woman):
        nra +=1
        if nra - ageAtIndex+1 >= 0:
            qualificationInc = adjusted_income_stream[0:nra - ageAtIndex+1]
        else:
    ...
    highest_indices_qualification = np.argsort(adjusted_income_stream)[-highest_years_qualification:] #python is right exclusive
    ...
    #print(total_wage)
    #print(count)
    avg_monthly_wage_A= max(total_wage_entitlement/(max(24,count*12)), total_wage_qualification/(max(24, (highest_years_qualification)* 24)))
...
def q1956(adjusted_income_stream:list, original_income_stream:list, index_year:int, birth_year:int,  retirement_year:int,  woman:bool): #this might not be completely correct/ might be off by half a year
...
def qcurrent1956(income_stream:list, index_year:int, birth_year:int, retirement_year:int, reference_year:int, death_year:int, woman:bool): 
...
def b1939helper(income_stream:list, index_year:int, birth_year:int, retirement_year:int, woman:bool):
    ...
    for year in range(index_year, index_year + len(income_stream)):
        ...
        if q1956(incomeStream, index_year, birth_year, retirement_year,woman) and yearOfQual  == 0: yearOfQual = year
...
#found on pages 11 and 12 of pdf; these are very complicated and arcane. I doubt that, as of feb 5 2025, they are completely correct of particular concern is the paragraph immediately following the table, these are the same as 1954
#benefit rules
#there is also a 1953 rule I am missing on pdf page 11
def  bb1956(adjusted_income_stream:list,  original_income_stream:list, avg_monthly_wage:float, index_year:int, birth_year:int, retirement_year:int, reference_year:int, woman:bool):
    ...
    pib=0 
    pia_1952=0
    # avg_monthly_wage=a1956(adjusted_income_stream, index_year, birth_year, woman, reference_year, woman)
    #print("avg: ", avg_monthly_wage)
    if (birth_year+65<1950 and q1939(i1939(original_income_stream, index_year, retirement_year), index_year, birth_year, retirement_year,woman)):# and index_year <= 1950): #this checks if they are old enough to have a pib; someone 
        #pib = round(b1939helper(adjusted_income_stream, index_year, birth_year,retirement_year, woman))
    ...
    pia_1952_list = [
        ...
        [65.50, 66.60], [66.60, 67.80], [67.80, 69.00], [68.90, 70.00], [70.00, 71.00], [71.00, 73.00], 
        [72.00, 74.00], [73.10, 75.10], [74.10, 76.10], [75.10, 77.10], [76.10, 78.10], [77.10, 77.20], 
        [77.20, 77.30], [77.30, 77.40], [77.40, 77.50], [77.50, 78.00], [78.00, 79.00], [79.00, 80.10], 
        [80.10, 81.00], [81.00, 82.00], [82.00, 84.00], [83.10, 84.00], [84.00, 86.00], [85.00, 86.00]
    ]
    ...
    pia_B=0
    pia_C=0
    #actual benefit formula, check this?
    if ((q1956(adjusted_income_stream, original_income_stream, index_year, birth_year, retirement_year,woman)==True) ): #Check if the worker is fully-insured, as well as to place them in the proper formula
        pia_A=(.55*min(avg_monthly_wage,110))+max((.20*min(avg_monthly_wage-110,240)),0)
    if ((q1956(adjusted_income_stream, original_income_stream, index_year, birth_year, retirement_year,woman)==True) ):
    ...
    if ((q1956(adjusted_income_stream, original_income_stream, index_year, birth_year, retirement_year,woman)==True) ):
        for i in range(len(pia_1952_list)):
            #print(pia_1952_list[i][0], pia_1952,pia_1952_list[i][1])
...
def b1956(income_stream:list, original_income_stream:list, index_year:int, birth_year:int, retirement_year:int, reference_year:int, woman:bool):
    ...
    adjusted_income_stream=i1956(income_stream, index_year, retirement_year)
    if not q1956(adjusted_income_stream, original_income_stream, index_year, birth_year, retirement_year, woman): return 0
    nominal_total_contributions=t1956(adjusted_income_stream, index_year)
    if retirement_year <= 1950: avg = a1939(i1939(original_income_stream, index_year, retirement_year), original_income_stream, index_year, birth_year, retirement_year, reference_year, woman)
    elif retirement_year <= 1952 : avg = a1950(i1950(original_income_stream, index_year, retirement_year), original_income_stream, index_year, birth_year, retirement_year, reference_year, woman)
    elif retirement_year <= 1954: avg = a1952(i1952(original_income_stream, index_year, retirement_year), original_income_stream, index_year, birth_year, retirement_year, reference_year, woman)
    elif retirement_year <= 1956: avg=a1954(i1954(original_income_stream, index_year, retirement_year),original_income_stream,  index_year, birth_year, retirement_year, reference_year, woman)
    else: avg = a1956(adjusted_income_stream, original_income_stream, index_year, birth_year, retirement_year, reference_year, woman)


    monthly_benefits=bb1956(adjusted_income_stream, original_income_stream, avg, index_year,birth_year, retirement_year, reference_year,woman)  


    #deductions for retiring early for women, page 3 of 1956 pdf
    nra=65
...
#tax rules, these are found on page 39 of the 1956 pdf
def t1956(income_stream:list, index_year:int):
    """This function outputs the nominal monthly benefit (int) an individual would have received under the 1950 legislation. 
        The function inputs are (income_stream:list, index_year:int, birth_year:int).
        income_stream -- a list of nominal incomes a person received in each year.
        index_year -- the year in which an individual begins their income_stream.
    ...
    adjusted_income_stream=i1956(income_stream, index_year, index_year + len(income_stream)) #to apply the taxable maximums to the benefit stream
    ...
    for i in range(len(adjusted_income_stream)): 
        ...
        if (index_year+i) <= 1956 and (index_year+i) >=1954:
             ...
             nominal_contributions.append(adjusted_income_stream[i]*employee_tax_rate)
        if (index_year+i) <= 1959 and (index_year+i) >=1957:
             employee_tax_rate= 0.0225
             employer_tax_rate= 0.0225
             nominal_contributions.append(adjusted_income_stream[i]*employee_tax_rate)
        if (index_year+i) <=1963 and (index_year+i)>=1960:
             employee_tax_rate= 0.0275
             employer_tax_rate= 0.0275
             nominal_contributions.append(adjusted_income_stream[i]*employee_tax_rate)
        if (index_year+i) <=1969 and (index_year+i)>=1964: 
             employee_tax_rate= 0.0325
             employer_tax_rate= 0.0325
             nominal_contributions.append(adjusted_income_stream[i]*employee_tax_rate)
        if (index_year+i) <=1974 and (index_year+i) >=1970: 
...
def death1956(income_stream:list, index_year:int, birth_year:int, retirement_year:int, reference_year:int,death_year:int, woman:bool):
    '''
    ...
    '''
    if (q1956(income_stream, index_year, birth_year, retirement_year,woman)): 
        monthlyBenefits = b1956(income_stream, index_year, birth_year, retirement_year, woman)
        return max(3 * monthlyBenefits, 255)
    else:
        return 0

def spouse1956(income_stream:list, index_year:int, birth_year:int, retirement_year:int, reference_year:int, woman:bool, spouse_income_stream:list, spouse_index_year:int, spouse_birth_year:int, spouse_retirement_year:int, spouse_reference_year:int, spouse_woman:bool):
    """This function outputs the nominal monthly benefit (int) an individual would have received under the 1950 legislation. 
        The function inputs are (income_stream:list, index_year:int, birth_year:int).
        income_stream -- a list of nominal incomes a person received in each year.
        index_year -- the year in which an individual begins their income_stream.
    ...
    if q1956(spouse_income_stream, spouse_index_year, spouse_birth_year, retirement_year,woman):
        spouse_benefit = b1956(spouse_income_stream, spouse_index_year, spouse_birth_year, spouse_retirement_year, spouse_reference_year,spouse_woman)
    else: spouse_benefit = 0
    primary_adjusted_income_stream=i1956(income_stream, index_year, retirement_year)
    primary_avg_monthly_wage = a1956(primary_adjusted_income_stream, index_year, birth_year, retirement_year,reference_year, woman)
    primary_PIA = bb1956(income_stream, primary_avg_monthly_wage, index_year, birth_year, retirement_year,reference_year, woman)
...
def widow1956(income_stream:list, index_year:int, birth_year:int, retirement_year:int, reference_year:int, death_year:int, woman:bool, widow_income_stream:list, widow_index_year:int, widow_birth_year:int, widow_retirement_year:int, widow_reference_year:int, widow_woman:bool):
    """This function outputs the nominal monthly benefit (int) an individual would have received under the 1950 legislation. 
        The function inputs are (income_stream:list, index_year:int, birth_year:int).
        income_stream -- a list of nominal incomes a person received in each year.
        index_year -- the year in which an individual begins their income_stream.
    ...
    """ 
    if ((widow_retirement_year >= widow_birth_year + 65) or (widow_woman == True and widow_retirement_year >= widow_birth_year +62)) and q1956(income_stream,index_year,birth_year, retirement_year, woman):
        widow_benefit = 0
        primary_adjusted_income_stream=i1939(income_stream, index_year, retirement_year)
        primary_avg_monthly_wage = a1956(primary_adjusted_income_stream, index_year, birth_year,retirement_year, reference_year, woman)
        widowMonthlyBenefit = b1956(widow_income_stream, widow_index_year, widow_birth_year, widow_retirement_year, widow_reference_year, widow_woman)
        primaryPIA = bb1956(income_stream, primary_avg_monthly_wage, index_year, birth_year, retirement_year, reference_year ,woman)
...
def MFB1956(income_stream:list, index_year:int, birth_year:int, retirement_year:int, reference_year:int,death_year:int, woman:bool):
    """
    ...
    primary_adjusted_income_stream=i1956(income_stream, index_year, retirement_year)
    primary_avg_monthly_wage = a1956(primary_adjusted_income_stream, index_year, birth_year, retirement_year,reference_year, woman)
    primaryMonthlyBenefit = b1956(primary_adjusted_income_stream, index_year, birth_year, retirement_year, reference_year, woman)
...
def children1956(income_stream:list, index_year:int, birth_year:int, retirement_year:int, reference_year:int,death_year:int, woman:bool, child_birth_year:int):
    """
    ...
    return math.ceil(.5* primary_PIA*10)/10