...
#income stream modifer, taking into account the maximums, as well as returning the avg monthly wage, income streams are now dfs!, page 7 of pdf
import numpy as np
...
def i1958(income_stream:list, index_year:int, retirement_year:int):
...
#there are no changes from 1956, check 1954/56 amendment model for citation
#the drop rule also needs to be added, sec 215(b)
def a1958(adjusted_income_stream:list, original_income_stream:list, index_year:int, birth_year:int, retirement_year:int, reference_year:int, woman:bool):
...
###################
    ...
    if nra - ageAtIndex+1 >= 0:
        ...
        qualificationInc =[]
    while not q1958(qualificationInc, original_income_stream, index_year, birth_year, retirement_year, woman):
        nra +=1
        if nra - ageAtIndex+1 >= 0:
            qualificationInc = adjusted_income_stream[0:nra - ageAtIndex+1]
        else:
    ...
    elasped_years_entitlement=end_year-beginning_year
    #print("elapsed: ", elasped_years)
    highest_years_entitlement=elasped_years_entitlement-drops
    highest_years_qualification = qualification_year - beginning_year - drops

    #If you become entitled to benefits before 1958, then the "new start" calculation doens't appy. For these folks, you use earnings history. (Page 3)
    elasped_years_entitlement=end_year-beginning_year 
    elapsed_years_qualification = qualification_year - beginning_year

    highest_years_entitlement=elasped_years_entitlement-drops
    highest_yeares_qualification = elapsed_years_qualification - drops

    if highest_years_entitlement<2:
       highest_years_entitlement=2
    if highest_yeares_qualification<2:
       highest_yeares_qualification=2


    highest_indices_entitlement = np.argsort(adjusted_income_stream)[-highest_years_entitlement:] #python is right exclusive
    highest_indices_qualification = np.argsort(adjusted_income_stream)[-highest_yeares_qualification:]
    ...
    for i in range(startIndex, len(adjusted_income_stream)): 
        if (i+index_year)>=beginning_year and (i in highest_indices_entitlement):
            #print("included: ", adjusted_income_stream[i])
    ...
    #         if (i-index_year) > 0 and adjusted_income_stream[i-index_year]<200: # Check that it is a valid income index and they make lesss than 200. Does this even work as planned? I need to fix this??????
    #             yrBefore22andNotCovered += 1 # Add one to not covered

    count = count - yrBefore22andNotCovered 
    #print("total: ", total_wage)
    avg_monthly_wage_A=max(total_wage_entitlement/(max(24,count*12)), total_wage_qualification/(max(24,12*(highest_years_qualification))))
...
def q1958(adjusted_income_stream:list, original_income_stream:list, index_year:int, birth_year:int,  retirement_year:int, woman:bool): #this might not be completely correct/ might be off by half a year
    ...
    adjusted_income_stream = i1958(adjusted_income_stream, index_year, retirement_year)
    ...
    #print(2*((end_year)-reference_year))
...
def qcurrent1958(income_stream:list, index_year:int, birth_year:int, retirement_year:int, reference_year:int, death_year:int, woman:bool): 
...
#found on pages 1, 2, and 3 of pdf; finally, these seem less complicated than the 1954 version and more clear
#there is also a 1953 rule I am missing on pdf page 11
def bb1958(adjusted_income_stream:list, original_income_stream:list, avg_monthly_wage:float, index_year:int, birth_year:int, retirement_year:int, reference_year:int, woman:bool):
    ...
    if pib>45.60: #there is probably a maximum pib somewhere in the legislation
        pib=45.60
    if q1954(i1954(original_income_stream, index_year, retirement_year), original_income_stream, index_year, birth_year, retirement_year, woman):
        pia_1954=b1954(adjusted_income_stream, original_income_stream,index_year, birth_year, retirement_year, reference_year, woman)
    #These initialize the tables used in the pia formula in the cases of turning 22 before 1950
    pib_list = [
        [00.00, 10.00], [10.01, 10.48], [10.49, 11.00], [11.01, 11.48], [11.49, 12.00], [12.01, 12.48], 
        [12.49, 13.00], [13.01, 13.48], [13.49, 14.00], [14.01, 14.48], [14.49, 15.00], [15.01, 15.60], 
        ...
        [43.77, 44.44], [44.45, 44.88], [44.89, 45.61]
    ]
    
    
    pia_1954_list=[
        [00.00, 30.00], [30.10, 31.00], [31.10, 32.00], [32.10, 33.00], [33.10, 34.00],
        [34.10, 35.00], [35.10, 36.00], [36.10, 37.00], [37.10, 38.00], [38.10, 39.00], [39.10, 40.00],
    ...
    avg_mon_wage_list = [
        [0, 55], [55, 57], [57, 59], [59, 61], [61, 62], [62, 64], [64, 66], [66, 68],
    ...
    if (q1958(adjusted_income_stream, original_income_stream, index_year, birth_year, retirement_year,woman)==True): #Check if the worker is fully-insured, as well as to place them in the proper formula
        for i in range(len(avg_mon_wage_list)):
            if avg_monthly_wage>=avg_mon_wage_list[i][0] and avg_monthly_wage<avg_mon_wage_list[i][1]:
                pia_A=pia_list[i]
    if (q1958(adjusted_income_stream, original_income_stream, index_year, birth_year, retirement_year,woman)==True and index_year <1951): #this not than (birth_year+65<=1950) because those retiring too early under the new start system would not work enough covered years for new start benefits
        for i, pibBonds in enumerate(pib_list):
            if pib >= pibBonds[0] and pib <= pibBonds[1]:
                pia_B = pia_list[i] #+ (pia_list[i+1] - pia_list[i])*(pib - pibBonds[0])  
    if (q1958(adjusted_income_stream, original_income_stream, index_year, birth_year, retirement_year,woman)==True): 
...
#total 
def b1958(income_stream:list, original_income_stream:list, index_year:int, birth_year:int, retirement_year:int, reference_year, woman:bool):
    ...
    adjusted_income_stream=i1958(income_stream, index_year, retirement_year)
    nominal_total_contributions=t1958(adjusted_income_stream, index_year)
    ...
    else: avg_monthly_wage=a1958(adjusted_income_stream, original_income_stream, index_year, birth_year, retirement_year, reference_year, woman)

    monthly_benefits=bb1958(adjusted_income_stream,original_income_stream, avg_monthly_wage, index_year, birth_year, retirement_year, reference_year, woman)
    
    nra=65
    if (retirement_year-birth_year)<nra and woman==True:
        early_deduction=12*0.00556*(nra-(retirement_year-birth_year))
        monthly_benefits=(1-early_deduction)*monthly_benefits
    elif woman==False and (retirement_year-birth_year)<nra:
        monthly_benefits=0
    
    return math.ceil(monthly_benefits * 10) / 10.0

#tax rules, found on page 30 of the 1958 pdf. Note that they report full OASDI taxes, not just OASI, so DI must be subtracted from them.
#note that disability insurance taxes is now active, these can be found on page 14 of 1956 pdf
def t1958(income_stream:list, index_year:int):
    
    employee_tax_rate = 0
    employer_tax_rate = 0
    nominal_contributions=[]

    adjusted_income_stream=i1958(income_stream, index_year, index_year + len(income_stream)) #to apply the taxable maximums to the benefit stream
    ...
    for i in range(len(income_stream)): 
        ...
        if (index_year+i) == 1959 :
             ...
             disability_tax=.005
             nominal_contributions.append(adjusted_income_stream[i]*employee_tax_rate)
        if (index_year+i) <=1962 and (index_year+i) >=1960:
        ...
        if (index_year+i) <=1965 and (index_year+i) >=1963: 
             ...
             disability_tax=.005
             nominal_contributions.append(adjusted_income_stream[i]*employee_tax_rate)
        if (index_year+i) <=1968 and (index_year+i) >=1966: 
             employee_tax_rate= 0.0375
             employer_tax_rate= 0.0375
             disability_tax=.005
             nominal_contributions.append(adjusted_income_stream[i]*employee_tax_rate)
        if (index_year+i) >=1969: 
             employee_tax_rate= 0.0425
             employer_tax_rate= 0.0425
             disability_tax=.005
             nominal_contributions.append(adjusted_income_stream[i]*employee_tax_rate)

    return nominal_contributions

def death1958(income_stream:list, index_year:int, birth_year:int, retirement_year:int, reference_year:int,death_year:int, woman:bool):
    '''
    ...
    '''
    if (q1958(income_stream, index_year, birth_year, retirement_year, woman)): 
        monthlyBenefits = b1958(income_stream, index_year, birth_year, retirement_year, woman)
        return max(3 * monthlyBenefits, 255)
    else:
        return 0
    
def spouse1958(income_stream:list, index_year:int, birth_year:int, retirement_year:int, reference_year:int, woman:bool, spouse_income_stream:list, spouse_index_year:int, spouse_birth_year:int, spouse_retirement_year:int, spouse_reference_year:int, spouse_woman:bool):
    """This function outputs the nominal monthly benefit (int) an individual would have received under the 1950 legislation. 
        The function inputs are (income_stream:list, index_year:int, birth_year:int).
        income_stream -- a list of nominal incomes a person received in each year.
        index_year -- the year in which an individual begins their income_stream.
    ...
    spousal_benefit = 0
    if q1958(spouse_income_stream, spouse_index_year, spouse_birth_year, spouse_retirement_year, spouse_woman):
        spouse_benefit = b1958(spouse_income_stream, spouse_index_year, spouse_birth_year, spouse_retirement_year, spouse_reference_year,spouse_woman)
    else: spouse_benefit = 0
    primary_adjusted_income_stream=i1958(income_stream, index_year, retirement_year)
    primary_avg_monthly_wage = a1958(primary_adjusted_income_stream, index_year, birth_year, retirement_year,reference_year, woman)
    primary_PIA = bb1958(income_stream, primary_avg_monthly_wage, index_year, retirement_year, birth_year,reference_year, woman)

    deduction = 1

    if spouse_woman == True:
...
def widow1958(income_stream:list, index_year:int, birth_year:int, retirement_year:int, reference_year:int,death_year:int,  woman:bool, widow_income_stream:list, widow_index_year:int, widow_birth_year:int, widow_retirement_year:int, widow_reference_year:int, widow_woman:bool):
    """This function outputs the nominal monthly benefit (int) an individual would have received under the 1950 legislation. 
        The function inputs are (income_stream:list, index_year:int, birth_year:int).
        income_stream -- a list of nominal incomes a person received in each year.
        index_year -- the year in which an individual begins their income_stream.
    ...
    """ 
    if ((widow_retirement_year >= widow_birth_year + 65) or (widow_woman == True and widow_retirement_year >= widow_birth_year +62)) and q1958(income_stream,index_year,birth_year, retirement_year,woman):
        if q1958(widow_income_stream,widow_index_year, widow_birth_year, widow_retirement_year, widow_woman):
            widowMonthlyBenefit = b1958(widow_income_stream, widow_index_year, widow_birth_year,widow_retirement_year, widow_reference_year, widow_woman)
        else: widowMonthlyBenefit = 0
        primary_adjusted_income_stream=i1958(income_stream, index_year, retirement_year)
        primary_avg_monthly_wage = a1958(primary_adjusted_income_stream, index_year, birth_year, retirement_year,reference_year, woman)
    
        primaryPIA = bb1958(income_stream,primary_avg_monthly_wage, index_year, birth_year, retirement_year, reference_year, woman)
...
def MFB1958(income_stream:list, index_year:int, birth_year:int, retirement_year:int, reference_year:int,death_year:int, woman:bool):
    """
    ...
    primary_adjusted_income_stream=i1958(income_stream, index_year, retirement_year)
    primary_avg_monthly_wage = a1958(primary_adjusted_income_stream, index_year, birth_year, retirement_year,reference_year, woman)
    primaryMonthlyBenefit = bb1958(primary_adjusted_income_stream, primary_avg_monthly_wage, index_year, birth_year, retirement_year, reference_year, woman)

    primary_avg_monthly_wage = math.floor(primary_avg_monthly_wage)
    #print(primary_avg_monthly_wage)
...
def children1958(income_stream:list, index_year:int, birth_year:int, retirement_year:int, reference_year:int,death_year:int, woman:bool, child_birth_year:int):
    """
    ...
    primary_adjusted_income_stream=i1958(income_stream, index_year, retirement_year)
    primary_avg_monthly_wage = a1958(primary_adjusted_income_stream, index_year, birth_year, retirement_year,reference_year, woman)
    primary_PIA = bb1958(income_stream, primary_avg_monthly_wage, index_year, birth_year, retirement_year,reference_year, woman)
