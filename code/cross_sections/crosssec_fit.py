...
# ----------------------------------------------------------------------------- data
def load_earnings(year, sex):
    q = ("SELECT a.earnings FROM annual a JOIN demographic d USING(id) "
         f"WHERE a.year={int(year)} AND d.sex={int(sex)} AND a.earnings>0")
...
def fit_dpln(x, lowc, highc):
    ...
    res = minimize(negll, [np.log(a0), np.log(b0), nu0, np.log(tau0)], method="L-BFGS-B")
