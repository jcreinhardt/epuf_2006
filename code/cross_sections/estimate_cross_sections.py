...
def main(jobs=None, rho=None, rho_steps=RHO_STEPS):
...
if __name__ == "__main__":
    kw = {}
    if "--jobs" in sys.argv:      kw["jobs"] = int(sys.argv[sys.argv.index("--jobs") + 1])
    if "--rho" in sys.argv:       kw["rho"] = float(sys.argv[sys.argv.index("--rho") + 1])
    if "--rho-steps" in sys.argv: kw["rho_steps"] = int(sys.argv[sys.argv.index("--rho-steps") + 1])
    main(**kw)
