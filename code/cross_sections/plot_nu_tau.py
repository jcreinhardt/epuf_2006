...
def plot(df, lims, out):
    with plt.rc_context(SLIDE_RC):
        fig, axes = plt.subplots(1, 2, figsize=(8.6, 3.18))
        for ax, (col, label, cmap) in zip(axes, PANELS):
            ...
            ax.set_ylabel("age")
            # at slide type the default decade/age ticks crowd each other
            ax.xaxis.set_major_locator(MaxNLocator(nbins=4, integer=True, steps=[2, 4, 5, 10]))
            ax.yaxis.set_major_locator(MaxNLocator(nbins=4, integer=True, steps=[2, 5, 10]))
        fig.tight_layout()
